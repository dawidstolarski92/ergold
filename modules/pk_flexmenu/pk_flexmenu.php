...
class pk_flexmenu extends Module {
	...
	public function __construct() {
		...
		parent::__construct();
		$this->displayName = 'FlexMenu';
		$this->description = 'Alysum Flexible Menu';
		$this->templateFile = 'module:pk_flexmenu/pk_flexmenu.tpl';
	}
			...
			foreach ($links as $link) {
				...
				$this->_html .= '
					<div class="list_menuitem">
						'.(isset($link['active']) && ($link['active'] == 1) ? '<img src="'.$this->_path.'assets/img/act.png" />' : ' <img src="'.$this->_path.'assets/img/pas.png" />').'<span>'.Tools::safeOutput($itemData['name']).'</span>
	...
	public function displayAddTabForm($id_menu = NULL) {
		...
		$id_shop = (int)Shop::getContextShopID();	
		$id_lang = (int)Context::getContext()->language->id;	
		$this->context->controller->addJS(_MODULE_DIR_. $this->name . '/assets/js/itemmanager-main.js');
			...
			<div class="after-label-add-content'.(isset($link_params['label']) ? '' : ' hide').'">
				...
				<div class="margin-form activeLabel">
					<div id="main_image_upload" data-type-content="image">
						...
						<img id="main_image_prev" src="'.(isset($link_params["main_image"]) && $link_params["main_image"] != "" ? $this->_path.'uploads/'.$link_params["main_image"] : $this->_path.'assets/img/demo.jpg').'" alt="" class="mainimagepreview previewImage"/>						
						...
						<div class="cleft_panel cpanel'.(isset($section_settings["state_left"]) && ($section_settings["state_left"] == 1) ? ' active' : '').'" data-section-name="left">
							...
							<div id="left-panel-container" class="panel-container">
								...
								<div id="left_image_upload" class="margin_field '.(isset($section_settings["left"]) && ($section_settings["left"] == "IMAGE") ? '' : 'hide').'" data-type-content="image">
									...
									<img id="left_image_prev" src="'.(isset($section_settings['left_image']) && $section_settings['left_image'] != "" ? $this->_path.'uploads/'.$section_settings['left_image'] : $this->_path.'assets/img/demo.jpg').'" alt="" class="leftimagepreview previewImage"/>
								...
								<div id="maincmspages" class="margin_field cms_p-container '.(isset($section_settings["main"]) && ($section_settings["main"] == "CMS_P") ? '' : 'hide').'" data-type-content="main-cms_pages">
									...
									<select id="maincmsp" data-dbname="main_cmsp">
										...
										//if (isset($section_settings["main_cmsp"]))
											$this->getCMSOptions(0, 1, $id_lang, (isset($link_params['label']) ? $link_params['label'] : ''), (int)Shop::getContextShopID());
							...
							<div id="right-panel-container" class="panel-container">
								...
								<div id="right_image_upload" class="margin_field '.(isset($section_settings["right"]) && ($section_settings["right"] == "IMAGE") ? '' : 'hide').'" data-type-content="image">
									<form method="post" enctype="multipart/form-data"  action="'.$this->_path.'ajax/upload.php">  
						       		...
						       		<strong>'.$this->l('Recommended image width is 180px').'</strong>	
									<img id="right_image_prev" src="'.(isset($section_settings["right_image"]) && $section_settings["right_image"] != "" ? $this->_path.'uploads/'.$section_settings["right_image"] : $this->_path.'assets/img/demo.jpg').'" alt="" class="rightimagepreview previewImage"/>
						...
						<div class="cbottom_panel cpanel'.(isset($section_settings["state_bottom"]) && ($section_settings["state_bottom"] == 1) ? ' active' : '').'" data-section-name="bottom">
							...
							<div id="bottom-panel-container" class="panel-container">
								...
								<div id="bottom_image_upload" class="margin_field '.(isset($section_settings["bottom"]) && ($section_settings["bottom"] == "IMAGE") ? '' : 'hide').'" data-type-content="image">
									...
									<img id="bottom_image_prev" src="'.(isset($section_settings["bottom_image"]) && $section_settings["bottom_image"] != "" ? $this->_path.'uploads/'.$section_settings["bottom_image"] : $this->_path.'assets/img/demo.jpg').'" alt="" class="bottomimagepreview previewImage"/>
	...
	private function getCategory($id_category, $depth = 0, $id_lang = false, $id_shop = false) {
		...
		if (!empty($is_intersected)) {
			...
			$this->_menu .= '<a class="menu-item-title" href="'.$category_link.'">';
			if ($depth == 0) {
				if (file_exists(_PS_IMG_DIR_.'c/'.$category->id.'.jpg')) {
				$this->_menu .= '<div class="fm-category-thumb"><img src="'.$category->image.'" width="200" height="100" alt="Category Image"/></div>';
				}
				$tag = 'h5';
			} else {
				$tag = 'div';
			}
			$this->_menu .= '<'.$tag.' class="cat-title">'.$category->name.'</'.$tag.'>';
			$this->_menu .= '</a>'.(count($children) ? '<a href="#" class="opener"></a>' : '');
			...
			switch ($type)
			{
				...
				case 'MAN':
					...
					if (!is_null($manufacturer->id))
					{
						...
						$link = new Link;
						$this->_menu .= '<li'.$selected.' class="menu-manufacturer"><a class="smooth02" href="'.$link->getManufacturerLink((int)$id, $manufacturer->link_rewrite).'"><img src="//'.$link->getManufacturerImageLink($manufacturer->id).'" alt="'.htmlspecialchars($manufacturer->name).'" title="'.htmlspecialchars($manufacturer->name).'" width="168" height="100" /></a></li>'.PHP_EOL;
					}
					break;
				...
				case 'BLG':
					...
					$url = $this->myRealUrl();
					if ($id == 0) {
						$this->_menu .= '<li class="blog-item"><a href="'.$url.$dispatcher->createUrl('module-ph_simpleblog-list', $id_lang, $params).'">'.$link['name'].'</a></li>'.PHP_EOL;
					} else {
						if ($links != false) {
							foreach ($links as $link) {
	...
	private function getProductImage($id)
	{
		...
		if (!is_null($product->id))
		$this->_menu .= '<li><div class="li-indent"><a class="product-image-link" href="'.htmlentities($product->getLink()).'"><img src="'.$this->context->link->getImageLink($product->link_rewrite, (int)$id.'-'.$cover['id_image'], 'home_'.$this->imageSuffix).'" alt="'.$product->name.'" /></a>';
		if ($product->show_price && !(bool)$pscatmode){
		$this->_menu .= '<a href="'.htmlentities($product->getLink()).'" class="menu-product-name ellipsis">'.$product->name.'</a><span class="price-container'.(($reduction) ? " reduction" : "").'"><span class="price">'.( !$priceDisplay ? Tools::displayPrice(Tools::ps_round($product->price*(0.01*$product->tax_rate)+$product->price, 2)) : Tools::displayPrice(Tools::ps_round($product->price, 2))).'</span>'.(($reduction) ? ' <span class="old_price">'.Tools::displayPrice($product->getPriceWithoutReduct(false)).'</span>' : '').'</span>'.PHP_EOL;}
	...
	{	
		...
		foreach ($data as $link) {			
			...
			$this->_menu .= '<li class="flexmenuitem dd_el'.((isset($itemData["home"]) && $itemData["home"] == true)  ? " home" : "").' id_menu'.$link["id_pk_flexmenu_menus"].(($submenu) ? " hasChildren" : "").(($link["narrow"] == 1)  ? " narrow" : "").(($this->curPath() == $itemData["link"]) ? " current-item" : "").'"><a href="'.$itemData['link'].'" '.(((!empty($itemData['new_window'])) && ($itemData['new_window'] == 1)) ? ' target="_blank"' : '').'><span>'.$itemData['name'].'</span></a>'.(($submenu) ? '<a href="#" class="opener"></a>' : '').'';		
						
			if ($submenu) {
			$this->_menu .='<div class="submenu dd_container'.(($link['state_left'] == 1) ? ' left-col-true' : '').' fmsid'.$link['id_pk_flexmenu_menus'].' clearfix" '.(($link['main_image'] != "")  ? ' style="background-image:url(/modules/pk_flexmenu/uploads/'.$link['main_image'].')"' : '').'><div class="menu_width"><div class="menu-wrapper clearfix">';
				...
				if ($link['state_left'] == 1) {
					...
					switch ($link['left'])	{								
						case 'PRD':				
							...
							$this->_menu .='<div class="left-title menu-product-title"><h3>'.$link['left_products_title'].'</h3></div>';
						...
						if (isset($link['left_image']) && !empty($link['left_image']))
							$this->_menu .='<a href="'.$link['left_image_link'].'" class="imagelink"><img src="'.$this->_path.'uploads/'.$link['left_image'].'?rev='.$rev.'" class="leftimage menu-image" alt="" /></a>';
						break;				
						case 'VID':
				...
				$this->_menu .='<div class="sections-contanier'.(($link['state_left'] == 0) ? ' full-width' : '').'">';
				...
				if ($link['state_main'] == 1) {
					...
					switch ($link['main']) {
						...
						case 'CMS_P':
							if ($link['main_cmsp']) {
								$this->_menu .='<div class="main-section-cmsp"><div class="page-width">';
								...
								$replace = array('[' => '<', ']' => '>');
        						$content = str_replace(array_keys($replace), array_values($replace), $cms->content);
				...
				if ($link['state_right'] == 1) {
					...
					switch ($link['right']) {				
						case 'PRD':
							$this->_menu .='<div class="right-title menu-product-title"><h3>'.$link['right_products_title'].'</h3></div>';
	...
	public function hookDisplayHeader($params) {
		$this->context->controller->addCss($this->_path.'assets/css/pk_flexmenu.css');
		//$this->context->controller->addCss($this->_path.'assets/css/pk_flexmenu_responsive.css');
		$this->context->controller->addJS($this->_path.'assets/js/pk_flexmenu.js');
	}
