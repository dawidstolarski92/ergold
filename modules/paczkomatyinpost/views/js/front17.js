...
function bind_carrier_radio() {
	uncheck_cgv();
	// initPaczkomatySelect($(".delivery-option input[type=radio]:checked"), true);
	initPaczkomatySelect($("#shipping-method [type=radio]:checked"), true);
}
...
// console.log(id);
	...
	target = $("#hook-display-after-carrier").first();
...
function bind_cgv(e) {
	...
	id = parseInt($(".delivery-option input[type=radio]:checked").val());
	if (paczkomatyinpost.opc_enabled === 1)
