...
if(isset($_POST) && !empty($_POST)){
	...
	if(isset($results['error']) && !empty($results['error'])){
		...
		if($commentobj->add()){
			...
			$results['success'][] = Context::getContext()->getTranslator()->trans("Comment Successfully Added", array(), 'Modules.xipblog.Admin');
			$results['results'] = $commentobj;
			die(Tools::jsonEncode($results));
		}else{
			$results['error'][] = Context::getContext()->getTranslator()->trans("Something went wrong please try again! ", array(), 'Modules.xipblog.Admin');
			die(Tools::jsonEncode($results));	
		}
	}
}