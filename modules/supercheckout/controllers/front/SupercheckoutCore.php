...
class SupercheckoutCore extends ModuleFrontController
{
    ...
    public function setMedia()
    {
        ...
        foreach ($css as $css_uri) {
            if ($uri = $this->getAssetUriFromLegacyDeprecatedMethod($css_uri)) {
                $this->registerStylesheet(sha1($uri), $uri, array('media' => 'all', 'priority' => 0));
            }
        }

        foreach ($js as $js_uri) {
            if ($uri = $this->getAssetUriFromLegacyDeprecatedMethod($js_uri)) {
        ...
        if ($this->context->cart->isVirtualCart()) {
            ...
            if ((int) $id_address_delivery > 0) {
                ...
                if ($this->checkout_session->getIdAddressDelivery() == 0) {
                    ...
                    $delivery_address->alias = $this->module->l('Title Delivery Alias')
                        . ' - ' . date('s') . rand(0, 9);
    ...
    public function createFreeOrder()
    {
        ...
        $free_order_error = $this->module->l('Free order');
        $order->validateOrder(
    ...
    protected function supercheckoutUpdateMsg($message_content)
    {
        if ($message_content) {
            if (!Validate::isMessage($message_content)) {
                $invalid_message_error = $this->module->l('Invalid message');
    ...
    protected function updateCarrier()
    {
        ...
        if (Tools::getIsset('delivery_option')) {
            ...
            $error[] = $this->module->l('Please select delivery option.');
        }
    ...
    protected function confirmOrder()
    {
        ...
        if (!$this->context->cart->isVirtualCart()) {
            if (!isset($posted_data['delivery_option']) || empty($posted_data['delivery_option'])) {
                $response['error']['checkout_option'][] = array('key' => 'shipping_method_error',
                    'error' => $this->module->l('No Delivery Method Selected.'));
                return $response;
            }
        }
        ...
        if (!isset($posted_data['payment_method']) && $order_total != 0) {
            $response['error']['general'][] = $this->module->l('Nie wybrano metody płatności.');
            return $response;
        }
        ...
        if (!$this->context->cart->checkQuantities()) {
            $msg = $this->module->l('An item in your cart is no longer available in this quantity.
        ...
        if ($this->context->cart->getOrderTotal(false, Cart::ONLY_PRODUCTS) < $minimal_purchase) {
            $msg = $this->module->l('A minimum purchase total of %1s (tax excl.) is required in
                order to validate your order, current purchase is %2s (tax excl.).');
        ...
        if (isset($posted_data['use_for_invoice'])) {
            ...
            if (isset($this->context->cookie->supercheckout_temp_address_invoice)
                ...
                $temp_invoice_address->alias = $this->module->l('Title Delivery Alias')
                    . ' - ' . date('s') . rand(0, 9);
        ...
        if ($shipping_address_value == 1) {
            foreach ($posted_data['shipping_address'] as $key => $value) {
                ...
                if ($add_plugin_config['conditional'] == 0
            ...
            if ($payment_address_value == 1) {
                foreach ($posted_data['payment_address'] as $key => $value) {
                    ...
                    if ($key == 'id_country') {
                        ...
                        if (isset($posted_data['payment_address']['postcode']) && $postcode_error) {
        ...
        if (!$this->is_logged) {
            ...
            if ($flag) {
                ...
                if (!$customer->add()) {
                    $response['error']['general'][] = $this->module->l('An error occurred
                    ...
                    if (!$customer->is_guest) {
                        ...
                        if (!$this->sendConfirmationMail($customer, $original_password)) {
                            $response['warning'][] = $this->module->l('An error ocurred while
        ...
        if (!isset($response['error'])) {
            if (Validate::isLoadedObject($delivery_address) && $delivery_address != null) {
                $delivery_address->id_customer = $id_customer;
                if (!$delivery_address->save()) {
                    $response['error']['general'][] = $this->module->l('Error occurred while updating address');
                } else {
                    $id_delivery_address = $delivery_address->id;
                }
            }
                ...
                if ($validation_type != '0') {
                    if (!Validate::$validation_type($field_value)) {
                        ...
                        if ($validation_type == 'isInt') {
                            $message = $this->module->l("Only digits are allowed.");
                        }
                        if ($validation_type == 'isName') {
                            $message = $this->module->l("Only name allowed.");
                        }
                        if ($validation_type == 'isEmail') {
                            $message = $this->module->l("Only email allowed (example@example.com).");
                        }
                        if ($validation_type == 'isDate') {
                            $message = $this->module->l("Only date is allowed. Check prestashop isDate() for reference.");
                        }
                        if ($validation_type == 'isUrl') {
                            $message = $this->module->l("Only URL is allowed.");
                        }

                        $errors['error_occured'] = 1;
                        $errors['error']["field_$id_velsof_supercheckout_custom_fields"] = $this->module->l("Please provide data in valid format") . $message;
                    }
                }
            }
        }
